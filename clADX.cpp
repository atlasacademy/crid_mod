...
{
	unsigned short r = v & 0xFF;
	r <<= 8;
	v >>= 8;
	r |= v & 0xFF;
	return r;
}
inline int bswap(int v)
{
	int r = v & 0xFF;
	r <<= 8;
	v >>= 8;
...
inline long long bswap(long long v)
{
	long long r = v & 0xFF;
	r <<= 8;
	v >>= 8;
...
{
	unsigned int i = bswap(*(unsigned int *)&v);
	return *(float *)&i;
}
...
bool clADX::Decode(const char *filename, const char *filenameWAV)
{
	...
	if (!Decode(fp2, &_header, sizeof(_header), 0))
	{
		fclose(fp2);
		fclose(fp);
		return false;
...
{
	int scale = bswap(*(unsigned short *)s);
	s += 2;
